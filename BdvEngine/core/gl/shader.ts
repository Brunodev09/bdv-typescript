  ...
  export abstract class Shader {
    ...
    public getAttribLocation(name: string): number {
      if (this.attributes[name] === null || this.attributes[name] === undefined)
        throw new Error(
          `Unable to fetch attr with name ${name} in shader ${this.shaderName}.`
        );
    ...
    public getUniformLocation(name: string): WebGLUniformLocation {
      if (this.uniforms[name] === null || this.uniforms[name] === undefined)
        throw new Error(
          `Unable to fetch uniform with name ${name} in shader ${this.shaderName}.`
        );
    ...
    private loadShader(source: string, shaderType: number): WebGLShader {
      ...
      if (error !== "") {
        throw new Error(
          `Error while compiling shader program with name ${this.shaderName}: ${error}`
        );
      }
    ...
    private createProgram(
      ...
      if (error !== "") {
        throw new Error(
          `Error linking shader with name ${this.shaderName}: ${error}`
        );
      }
    }

    private getAttributes(): void {
      let attrCount = gl.getProgramParameter(
        this.program,
