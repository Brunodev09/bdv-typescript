  ...
  export class MessageBus {
    ...
    public static subscribe(code: string, handler: IMessageHandler): void {
      ...
      if (MessageBus.subs[code].indexOf(handler) !== -1) {
        console.log(
          `MessageBus::Attempting to push duplicate handler to messaging code ${code}. No subscription added.`
        );
      } else {
    ...
    public static unsubscribe(code: string, handler: IMessageHandler): void {
      if (!MessageBus.subs[code]) {
        console.log(
          `MessageBus::There is no such handler subscribed to code ${code}.`
        );
        return;
      }
      if (MessageBus.subs[code].indexOf(handler) !== -1) {
    ...
    public static update(time: number): void {
      ...
      let limit = Math.min(
        MessageBus.queueMessageTick,
        MessageBus.messageQueue.length
      );
